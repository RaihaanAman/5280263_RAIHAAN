...
char** copy_grid(char** grid, int r, int c) {
    ...
    for (int i = 0; i < r; i++) {
        newg[i] = malloc((c + 1) * sizeof(char));
        strcpy(newg[i], grid[i]);
    }
    return newg;
}
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    *result_count = r;
    ...
    char** first_explode = explode(grid, r, c);
    if (n % 4 == 3) {
        return first_explode;
    } else {
        char** second_explode = explode(first_explode, r, c);
        for (int i = 0; i < r; i++) free(first_explode[i]);
        free(first_explode);
